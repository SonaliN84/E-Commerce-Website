...
const Header=(props)=>{
 ...
 const NumberOfItemsInCart=Authctx.data.reduce((curNumber,item)=>{
   ...
   return(
    ...
    <Navbar bg="dark" variant="dark" className='d-flex'>
     ...
     {Authctx.isLoggedIn && ( <Button variant="primary"  onClick={props.onShowModal}>
      Cart<Badge bg="dark" className='ms-2' >  {NumberOfItemsInCart}</Badge></Button>)} 
      {!Authctx.isLoggedIn &&  ( <Button variant="primary"  onClick={props.onShowModal}>
      Cart<Badge bg="dark" className='ms-2' >  0</Badge></Button>   )}
      {!Authctx.isLoggedIn &&  (
